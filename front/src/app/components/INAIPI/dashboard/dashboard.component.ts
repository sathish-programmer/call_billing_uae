...
export class DashboardComponent implements OnInit, AfterViewInit {
  ...
  callSummaryJson = {
    totalCalls: 0,
    incomingCalls: 0,
    incomingCallsDuration: 0,
    outgoingCalls: 0,
    outgoingCallsDuration: 0,
    missedCalls: 0,
    internationalOutgoingCalls: 0,
    averageTalkTime: 0,
    totalChargableCalls: 0,
    totalChargableCallsCost: 0,
    lastCallTime: '',
  };
  ...
  public barChartData: any[] = [
    { data: [0, 0, 0, 0, 0, 0, 0], label: 'Outgoing Call Summary' },
  ];
  ...
  public lineChartData: Array<any> = [
    {
      ...
      label: 'Outgoing Call Usage (X-Time, Y-Sec)',
    },
  ];
  ...
  //CHART 3
  public lineChartDataIncoming: Array<any> = [
    {
      data: [0, 0, 0, 0, 0, 0, 0, 0],
      label: 'Incoming Call Usage (X-Time, Y-Sec)',
    },
  ];
  public lineChartLabelsIncoming: Array<any> = [
    '',
    '10',
    '11',
    '12',
  ...
  callDateChangeEvent() {
    this._zone.run(() => {
      this.todayDateToShow = moment(this.savedDateChangeEvent).format(
        'MMM D, YYYY'
      );
      this.previousDateToShow = moment(this.savedDateChangeEvent)
        .subtract(1, 'days')
        .format('MMM D, YYYY');
      this.todayDateToUse = moment(this.savedDateChangeEvent).endOf('day');
      this.previousDateToUse = moment(this.savedDateChangeEvent)
        .subtract(1, 'days')
        .startOf('day');
    });
    this.getAllList();
  ...
  previousCallSummary(orgId) {
    ...
    this.authService.getCallSummary(orgId, dataToSend).subscribe((res) => {
      this.previousJson = this.bifurcateCallSummaryRelatedData(
        res['data'],
        false
      );
    });
  ...
  bifurcateCallSummaryRelatedData(data, callChartFunction) {
    ...
    resultJSON['missedCalls'] = _.where(logFileData, {
      CallDuration: 0,
    }).length;
    resultJSON['totalCalls'] = logFileData.length;

    if (logFileData && logFileData.length) {
      resultJSON['lastCallTime'] = moment(logFileData[0]['CallTime'])
        .utc(false)
        .format('LT');
    }
    ...
    if (allIncomingCalls && allIncomingCalls.length) {
      ...
      var incomingCallsDurationArray = _.difference(
        _.pluck(allIncomingCalls, 'CallDuration'),
        [NaN]
      );
      resultJSON['incomingCallsDuration'] = _.reduce(
        incomingCallsDurationArray,
        function (memo, num) {
          return memo + num;
        },
        0
    ...
    if (allOutgoingCalls && allOutgoingCalls.length) {
      ...
      var outgoingCallsDurationArray = _.difference(
        _.pluck(allOutgoingCalls, 'CallDuration'),
        [NaN]
      );
      resultJSON['outgoingCallsDuration'] = _.reduce(
        outgoingCallsDurationArray,
        function (memo, num) {
          return memo + num;
        },
        0
      ...
      resultJSON['internationalOutgoingCalls'] =
        this.filterTypeFromOutGoingData(allOutgoingCalls, 'international');
    }
    ...
    if (
      (resultJSON['incomingCallsDuration'] ||
        resultJSON['outgoingCallsDuration']) &&
      resultJSON['totalCalls']
    ) {
      resultJSON['averageTalkTime'] = (
        (resultJSON['incomingCallsDuration'] +
          resultJSON['outgoingCallsDuration']) /
        resultJSON['totalCalls']
      ).toFixed(2);
    ...
    var allCalculatedCostArray = _.difference(
      _.pluck(logFileData, 'CalculatedCost'),
      [NaN, undefined]
    );
    resultJSON['totalChargableCallsCost'] = _.reduce(
      allCalculatedCostArray,
      function (memo, num) {
        return memo + num;
      },
      0
  ...
  filterTypeFromOutGoingData(calls, type) {
    ...
    return returnCallsArray && returnCallsArray.length
      ? returnCallsArray.length
      : 0;
  }
  ...
  makeOutgoingCallChartSummary(outgoingCalls) {
    this._zone.run(() => {
      var data = [
        this.filterTypeFromOutGoingData(outgoingCalls, 'local'),
  ...
  makeCallChartUsage(allCalls, callType) {
    this._zone.run(() => {
      ...
      if (allCalls && allCalls.length) {
        ...
        var lastTime = moment(allTime[allTime.length - 1])
          .utc(false)
          .add(1, 'hours')
          .seconds(0)
          .minutes(0)
        ...
        var allTime: any[] = _.map(allTime, function (singleTime) {
          var formatedData = moment(singleTime)
            .utc(false)
            .seconds(0)
            .minutes(0)
            .milliseconds(0);
          var timeFromData = moment(formatedData)
            .utc(false)
            .format()
            .split('T');
          ...
          return {
            actualData: singleTime,
            formatData: formatedData['_d'],
        ...
        for (var index in formatData) {
          if (parseInt(index) < formatData.length) {
            if (
              String(formatData[index]) !=
              String(formatData[parseInt(index) + 1])
            ) {
              chartDataArray.push(
                this.filterDurationAsPerDateFromData(
                  allCalls,
                  formatData[index],
  ...
  filterDurationAsPerDateFromData(calls, now, later) {
    var returnCallsArray = _.filter(calls, function (singleJson) {
      if (
        moment(singleJson['CallTime']) >= now &&
  ...
  getCallListMissedWise(orgId, skipPage) {
    ...
    this.authService.getCallList(this.orgId, dataToSend).subscribe((res) => {
      for (let index in res['data']) {
        ...
        res['data'][index]['CallTime'] = moment(res['data'][index]['CallTime'])
          .utc(false)
          .format('l LT');
      }
  ...
  changeDateFormatForList(resData) {
    for (let index in resData) {
      resData[index]['CallTime'] = moment(resData[index]['CallTime'])
        .utc(false)
        .format('l LT');
    }
