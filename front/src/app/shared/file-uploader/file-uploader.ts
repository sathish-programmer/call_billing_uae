...
import { Component, ElementRef, NgZone, Output, EventEmitter } from '@angular/core';
...
changeListner(event) {
    if (!event.target.files[0].name.match(/\.(csv|CSV)$/)){
        ...
        this.toastr.error('File format not supported. Please select a valid CSV file.');
        return;
    }
...
save(){
    let fileUploaded:any = document.getElementById('imk_inputFileFromUploadImageModal_nocrop');
    ...
    if (!originalFile.name.match(/\.(csv|CSV)$/)){
        // this.toastr.error('File format not supported. Please select a valid image or PDF/Doc file.');
        this.toastr.error('File format not supported. Please select a valid image or CSV file.');
        return;
    }
    ...
    this._zone.run(()=>{
        ...
        this.authService.saveTariffFile(formdata).subscribe(res => {
            this.isUploading = false;
            this.status = "Upload";
