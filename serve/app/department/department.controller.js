    ...
    if (proceed) {
      ...
      if (body.name && body.name.toLowerCase().trim() == "all") {
        return res.json({
          success: false,
          data: "",
          message: "Department Name not allowed",
        });
      }
      ...
      if (body.name && body.organization && body.branch) {
        if (
          ...
          return res.json({
            ...
            data: "",
            message:
              "Department already exists in same organization and branch",
          });
        } else {
          let retDoc = new DEPARTMENT({
            name: body.name,
            organization: body.organization,
            branch: body.branch,
    ...
    if (proceed) {
      ...
      if (
        ...
        if (
          ...
          let retDoc = await DEPARTMENT.findByIdAndUpdate(
            { _id: params.departmentId, softDelete: false },
            {
              $set: {
                name: body.name,
  ...
  try {
    ...
    if (proceed) {
      ...
      let departmentList = await DEPARTMENT.find(
        dbQuery,
        "name organization creationDate branch",
        filterQuery
      )
      ...
      return res.json({
        success: false,
        data: "",
        message: "Not Authorized to list department",
      });
    }
  } catch (err) {
      ...
      if (params.departmentId) {
        ...
        if (proceed) {
          return res.json({
            success: false,
            data: "",
            message: "Sub-Department linked with Department.",
          });
        }
        ...
        if (proceed) {
          return res.json({
            success: false,
            data: "",
            message: "User linked with department.",
          });
        }
