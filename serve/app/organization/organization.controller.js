    ...
    if (proceed) {
      ...
      if (body.name && body.parent) {
        ...
        if (await ORG.findOne({name: body.name, softDelete: false})) {
          return res.json({success: false, data: "", message: "Organization with same name already exists"});
        } else {
          let retDoc = new ORG({name: body.name,
          ...
          let dpUserDoc = new DP_USER({ firstName: body.name,
                                        lastName: 'DP',
          ...
          if (doc) {
            return res.json({success: true, data: doc['_id'], message: "Organization Added"});
          } else {
...
exports.getOrganizationListForDropDown = async (req, res) => {
  try {
    ...
    if (proceed) {
      ...
      if (proceed) {
        let aggregateQuery = [{"$match": {"_id": mongoose.Types.ObjectId(req.user.organization)}},
                              {"$graphLookup": {from: "organizations",
                                                startWith: "$_id",
        ...
        return res.json({success: true, data: orgList, messagee: "List Found"});                            
      } else {
        let retDoc = await ORG.findOne({_id: req.user.organization, softDelete: false}, "name creationDate").lean();
        retDoc['child'] = [];

        return res.json({success: true, data: [retDoc], messagee: "List Found"});
      }
    } else {
      let retDoc = await ORG.findOne({_id: req.user.organization, softDelete: false}, "name creationDate").lean();
      retDoc['child'] = [];
  ...
  try {
    ...
    if (proceed) {
      ...
      if (body.name && params.orgId) {
        if (await ORG.findOne({name: body.name, _id: {"$ne": params.orgId}, softDelete: false})) {
          return res.json({success: false, data: "", message: "Organization with same name already exists"});
        } else {
          let retDoc = await ORG.findByIdAndUpdate( {_id: params.orgId, softDelete: false, type: 'normal'}, 
                                                    {"$set": {name: body.name,
                                                              description : body.description || '',
          ...
          if (retDoc) {
            return res.json({success: true, data: retDoc['_id'], message: "Organization Updated"});
          } else {
            return res.json({success: false, data: "", message: "Something went wrong"});
          }        
        }
    ...
    if (proceed) {
      ...
      if (proceed) {
        ...
        if (retDoc && retDoc.length) {
          function minimizedTree(treeFile) {
            orgList.push({name: treeFile['name'], _id: treeFile['_id'], 
                          parent: treeFile.parent, description: treeFile.description, 
        ...
        for (let index in orgList) {
          orgList[index]['child'] = await ORG.find({softDelete: false, parent: orgList[index]['_id']});
          orgList[index]['parentName'] = '';

          if (orgList[index]['parent']) {
            let org = await ORG.findOne({softDelete: false, _id: orgList[index]['parent']});
        ...
        let retDoc = await ORG.findOne({_id: req.user.organization, softDelete: false}, "name description creationDate parent").lean();
        
        if (retDoc) {
          let org = await ORG.findOne({softDelete: false, _id: retDoc['parent']});
  ...
  try {
    ...
    if (proceed) {
      if (params.orgId) {
        ...
        let proceed = await findOneOrganization({parent: params.orgId, softDelete: false});
        
        if (proceed) {
          return res.json({success: false, data: "", message: "Child Organization exists"});
        }

        proceed  = await findOneBranch(query);
        
        if (proceed) {
          return res.json({success: false, data: "", message: "Branch linked with organization."});
        }

        proceed  = await findOneDepartment(query);
        
        if (proceed) {
          return res.json({success: false, data: "", message: "Department linked with organization."});
        }

        proceed  = await findOneSubdepartment(query);
        
        if (proceed) {
          return res.json({success: false, data: "", message: "Sub-Department linked with organization."});
        }
        
        proceed  = await findOneRole(query);
        
        if (proceed) {
          return res.json({success: false, data: "", message: "Role linked with organization."});
        }

        proceed  = await findOneUser(query);
        
        if (proceed) {
          return res.json({success: false, data: "", message: "User linked with organization."});
        }

        proceed  = await findOneProvider(query);
        
        if (proceed) {
          return res.json({success: false, data: "", message: "Provider linked with organization."});
        }

        proceed  = await findOneTariff(query);
        
        if (proceed) {
          return res.json({success: false, data: "", message: "Tariff linked with organization."});
        }

        proceed  = await findOneAssignTariff(query);
        
        if (proceed) {
          return res.json({success: false, data: "", message: "Assign Tariff linked with organization."});
        }

        proceed  = await findOneCallLog(query);
        
        if (proceed) {
          return res.json({success: false, data: "", message: "Call Logs linked with organization."});
        }

        await ORG.findOneAndUpdate({_id: params.orgId, softDelete: false, type: 'normal'},
                                            {"$set": {softDelete: true}});
