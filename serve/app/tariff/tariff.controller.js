...
exports.saveTariffFromFile = async (req, res) => {
  try {
    ...
    var tariffDetail,
      tariffRateAndTimeDetail,
      errors = [],
      errorJson;

    let providerNames = _.difference(
      _.uniq(
        _.pluck(_.pluck(tariffCompleteDetails, "tariffDetail"), "provider")
      ),
      [undefined]
    );
    ...
    if (providerNames && providerNames.length) {
      providerRetDoc = await PROVIDER.find({
    ...
    for (var index in tariffCompleteDetails) {
      ...
      tariffRateAndTimeDetail =
        tariffCompleteDetails[index].tariffRateAndTimeDetail;
      errorJson = {
      ...
      let singleProvider = _.findWhere(providerRetDoc, {
        name: tariffDetail["provider"],
      });
      if (singleProvider) {
        var tariffDataToInsert = new TARIFF({
          organization: tariffDetail["organization"],
        ...
        var singleCurrencyDoc = _.findWhere(currencyMasterList, {
          symbol: tariffDetail["currencyName"],
        });

        if (singleCurrencyDoc) {
          tariffDataToInsert["currency"] = singleCurrencyDoc["_id"];
        } else {
          errorJson.errors.push({
            type: "Currency",
            detail: "Not able to find currency detail",
          });
        }

        var singleTimeZoneDoc = _.findWhere(timeZoneMasterList, {
          name: tariffDetail["timeZoneName"],
        });
        ...
        var details = await TARIFF.findOne(checkData, {
          sort: { creationDate: -1 },
        });
        ...
        if (details) {
          await setEndTime(
            details["_id"],
            tariffRateAndTimeDetail["rateStartDate"]
          );
        }

        var tariffId = await tariffDataToInsert.save();

        var tariffRateAndTimeDataToInsert = new TARIFF_RATE_TIME({
          organization: tariffRateAndTimeDetail.organization,
          tariffId: tariffId,
          ...
          specialRateStartDate: new Date(
            tariffRateAndTimeDetail["specialRateStartDate"]
          ),
          specialRateEndDate: new Date(
            tariffRateAndTimeDetail["specialRateEndDate"]
          ),
          creationDate: new Date(),
          createdBy: req.user._id,
        });
  ...
  try {
    ...
    if (proceed) {
      ...
      if (
        ...
        let retDoc = await TARIFF.findByIdAndUpdate(
          {
            _id: params.tariffId,
            organization: body.organization,
            softDelete: false,
...
exports.getSingleTariff = async (req, res) => {
  try {
    ...
    if (proceed) {
      if (params.tariffId) {
        let retDoc = await TARIFF.findOne({
          _id: params.tariffId,
          softDelete: false,
        });
        return res.json({
          success: true,
          data: retDoc,
          message: "Single Tariff",
        });
      } else {
        return res.json({
          success: false,
          data: "",
          message: "Tariff Id missing",
        });
      }
    } else {
      return res.json({ success: false, data: "", message: "Not authorized" });
    ...
    if (proceed) {
      ...
      if (params.tariffId) {
        await TARIFF.findByIdAndUpdate(
          { _id: params.tariffId, softDelete: false },
          { $set: { softDelete: true } }
        );
        await TARIFF_RATE_TIME.updateMany(
          { tariffId: params.tariffId, softDelete: false },
          { $set: { softDelete: true } }
        );
        return res.json({
          success: true,
        ...
        return res.json({
          success: false,
          data: "",
          message: "Missing Tariff Rate and time",
        });
      }
    } else {
      return res.json({ success: false, data: "", message: "Not Authorized" });
...
async function setFileErrors(errors, fileId) {
  await TARIFF_FILE.update({ _id: fileId }, { $set: { fileErrors: errors } });
}
