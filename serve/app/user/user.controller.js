    ...
    if (proceed) {
      if (
        ...
        if (
          ...
          let retDoc = await USER.findByIdAndUpdate(
            { _id: params.userId, softDelete: false, type: "normal" },
            { $set: userObject }
          );
      ...
      if (params.userId) {
        ...
        if (proceed) {
          return res.json({
            success: false,
            data: "",
            message: "Call Logs linked with user.",
          });
        }
  ...
  try {
    ...
    if (user && user.type == "root") {
      if (params && params.userId) {
        let userDoc = await DP_USER.findOne({
          _id: params.userId,
          softDelete: false,
        });
        ...
        if (userDoc) {
          ...
          await DP_USER.findByIdAndUpdate(
            { _id: userDoc["_id"], softDelete: false },
      ...
      return res.json({
        success: false,
        data: "",
        message: "Not Authorized to create token",
      });
    }
  } catch (err) {
    console.log("Error while deleting user", err);
...
exports.getDPUsers = async (req, res) => {
  try {
    ...
    if (params && params.orgId) {
      let dpUserList = await DP_USER.find({
        organization: params.orgId,
        softDelete: false,
      });
      return res.json({
