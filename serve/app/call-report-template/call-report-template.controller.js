...
exports.saveCallReportsTemplate = async (req, res) => {
  try {
    ...
    if (body.organization && body.startDate && body.endDate && body.fileName) {
      let dataToSave = new CALL_REPORT_TEMPLATE({"organization"    : body.organization,
                                                  "user"            : req.user._id,
...
exports.updateCallReportTemplate = async (req, res) => {
  try {
    ...
    if (body.organization && body.startDate && body.endDate && body.fileName) {
      let retDoc = await CALL_REPORT_TEMPLATE.findByIdAndUpdate({_id: params.saveCallReportsFilterId, 
                                                                    user: req.user._id, softDelete: false},
...
exports.getCallReportsTemplate = async (req, res) => {
  try {
    let retDocs = await CALL_REPORT_TEMPLATE.find({user: req.user._id, softDelete: false});
...
exports.deleteCallReportTemplate = async (req, res) => {
  try {
    ...
    if (params.saveCallReportsFilterId) {
      let retDocs = await CALL_REPORT_TEMPLATE.findByIdAndUpdate({_id: params.saveCallReportsFilterId, user: req.user._id, softDelete: false},
                                                                    {"$set": {softDelete: true}});
      return res.json({success: true, data: retDocs, message: "Saved"}); 
    } else {
      return res.json({success: false, data: "", message: "Missing Template Id"});
    }
  } catch (err) {
    console.log("Error while saving reports filter", err);
    return res.json({succes: false, data:"", message: err});
